  ...
  */
  ...
  public function doCreditCardPayment($params) {
    ...
    try {
      ...
      // TODO Get wsdl from config.
      $wsdl = "https://demo.bambora.co.nz/interface/api/dts.asmx?wsdl";
      $payload = '<Transaction>
                  <AccountNumber>' . $params["AccountNumber"] . '</AccountNumber>
                  <CustRef>' . $params["CustRef"] . '</CustRef>
                  <CustNumber>' . $params["CustNumber"] . '</CustNumber>
      ...
      // Initialize Soap Client and prepare parameters for call.
      $soap = new SoapClient($wsdl, ['trace' => 1]);
      // Execute API Call
      $data = $soap->SubmitSinglePayment(['trnXML' => $payload]);
      $response_soap = $soap->__getLastResponse();

      \Drupal::logger('ccc')->info(print_r($data, TRUE));
      \Drupal::logger('ccc soap last response')->info($response_soap);


      $xmlobj = preg_replace("/(<\/?)(\w+):([^>]*>)/", "$1$2$3", $response_soap);
      $array_data = strip_tags($xmlobj);
